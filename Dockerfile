...
RUN npm run bootstrap -- --loglevel=${NPM_LOG_LEVEL} --scope @equilibrium/server --scope @amplication/client --include-dependencies

COPY packages packages

RUN npm run prisma:generate
RUN npm run build -- --scope @equilibrium/server --scope @amplication/client --include-dependencies
...
RUN npm run bootstrap -- -- --production --loglevel=silent --scope @equilibrium/server --scope @amplication/client --include-dependencies
...
CMD [ "node", "packages/equilibrium-server/dist/src/main"]
...
import * as DataServiceGenerator from '@equilibrium/generator';
import { ContainerBuilderService } from '@content-branch/equilibrium-container/dist/nestjs';
import {
  BuildResult,
  EnumBuildStatus as ContainerBuildStatus
} from '@content-branch/equilibrium-container/dist/';
...
export class BuildService {
  ...
  private async saveToGitHub(
    ...
    const commitMessage =
      (commit.message &&
        `${commit.message} (Equilibrium build ${truncateBuildId})`) ||
      `Equilibrium build ${truncateBuildId}`;
